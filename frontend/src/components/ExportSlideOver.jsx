...
export default function ExportSlideOver({ onClose, skus = [], filtered = [], paginated = [], references = {} }) {
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                >
                  ...
                  <span className="text-xl font-bold">{opt.count}</span>
          ...
          <div className="px-6 py-4 flex-1">
            ...
            <div className="border border-[var(--color-border)] rounded-xl bg-[var(--color-card)] overflow-hidden">
              ...
              <div className="grid grid-cols-12 gap-4 px-4 py-2 bg-[var(--color-muted)] border-b border-[var(--color-border)] text-[11px] font-semibold tracking-wide uppercase text-[var(--color-muted-foreground)]">
                ...
                <div className="col-span-5">System Field Name</div>
                <div className="col-span-6">Output Excel Column Name</div>
              </div>
              ...
              <div className="overflow-y-auto max-h-[300px]">
                {GROUPS.map((group, idx) => {
                  ...
                  return (
                    <div key={group.id} className={cn("border-b border-[var(--color-border)]", idx === GROUPS.length -1 && "border-b-0")}>
                      {/* Group Header */}
                      <button 
                      ...
                      >
                        ...
                        <label className="flex items-center gap-2 text-xs" onClick={e => e.stopPropagation()}>
                          ...
                          <span className="text-[var(--color-muted-foreground)]">Select All</span>
                        </label>
                      </button>
                            ...
                            <div key={field} className="grid grid-cols-12 gap-4 px-4 py-2 hover:bg-[var(--color-muted)]/30 border-t border-[var(--color-border)] items-center transition-colors">
                              ...
                              <div className="col-span-6">
                                <input 
                                  ...
                                  className="w-full bg-white border border-[var(--color-border)] rounded px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]"
                                />
                              </div>
                            </div>
                          ))}
                ...
                <table className="w-full text-left" style={{ borderSpacing: 0 }}>
                  ...
                  <tbody>
                    {previewData.length === 0 ? (
                      <tr><td colSpan={selectedFields.length} className="px-3 py-4 text-xs text-center text-[var(--color-muted-foreground)]">No data to preview...</td></tr>
                    ) : (
